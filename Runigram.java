...
public class Runigram {
	...
	public static void main(String[] args) {
		...
		Color test = blend( new Color (100,40,100),new Color(200, 20 ,40), 0.25);
	...
	public static Color luminance(Color pixel) {
		//// Replace the following statement with your code
		double x = 0.299*pixel.getRed() + 0.587*pixel.getGreen() + 0.114*pixel.getBlue();
		int lum = (int) x;
	 ...
	 * The image is scal   ed (resized) to have the given width and height.
	 */
	...
	public static void morph(Color[][] source, Color[][] target, int n) {
		...
		if (source.length != target.length || source[0].length != target[0].length) {
			scaledimage  = scaled(target, source[0].length, source.length);
		} else {
			scaledimage = target;
		}
		...
		double a;
		while (n >= 0) {
			a = (double)n/denominator;
			morph = blend(source, scaledimage, a);
